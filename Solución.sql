...
SELECT *
FROM customers;
SELECT c.first_name, c.last_name, o.customer_id, count(o.order_id) AS "pedidos"
FROM orders o
JOIN customers c
...
SELECT oi.product_id, pt.product_name, sum(oi.quantity) AS "Vendidos", pt.price, sum(pt.price*oi.quantity) AS "Ingreso"
FROM order_items oi
...
SELECT o.order_date, sum(pt.price*oi.quantity) AS "Ingresos"
FROM orders o
JOIN order_items oi
...
SELECT o.customer_id, c.first_name, c.last_name, min(o.order_date)
FROM orders o
JOIN customers c
on o.customer_id = c.customer_id
GROUP BY customer_id;
...
SELECT oi.order_id, sum(pt.price*oi.quantity) AS "Ingreso",
CASE WHEN sum(pt.price*oi.quantity) > 300 THEN "Caro"
	 WHEN sum(pt.price*oi.quantity) BETWEEN 100 AND 300 THEN "Asequible"
     ELSE "Barato" END AS Segmentación
FROM order_items oi
JOIN products_tiny pt
ON oi.product_id = pt.product_id
